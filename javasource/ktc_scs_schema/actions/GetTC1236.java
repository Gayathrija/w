// This file was generated by Mendix Studio Pro.
//
// WARNING: Only the following code will be retained when actions are regenerated:
// - the import list
// - the code between BEGIN USER CODE and END USER CODE
// - the code between BEGIN EXTRA CODE and END EXTRA CODE
// Other code you write will be lost the next time you deploy the project.
// Special characters, e.g., é, ö, à, etc. are supported in comments.

package ktc_scs_schema.actions;

import com.mendix.systemwideinterfaces.core.IContext;
import com.mendix.systemwideinterfaces.core.IMendixObject;
import com.mendix.webui.CustomJavaAction;
import ktc_scs_schema.proxies.TC1236_KNTRSK35DS;
import ktcmdwscsconnector.customs.MessageClass;

public class GetTC1236 extends CustomJavaAction<java.util.List<IMendixObject>>
{
	private java.lang.String messageString;
	private java.lang.String searchKey;
	private java.lang.String moreRecord;

	public GetTC1236(IContext context, java.lang.String messageString, java.lang.String searchKey, java.lang.String moreRecord)
	{
		super(context);
		this.messageString = messageString;
		this.searchKey = searchKey;
		this.moreRecord = moreRecord;
	}

	@java.lang.Override
	public java.util.List<IMendixObject> executeAction() throws Exception
	{
		// BEGIN USER CODE
		MessageClass msgClass = new MessageClass();
		java.util.List<IMendixObject> list = new java.util.ArrayList<IMendixObject>();
		
		try
		{
			// Start Detail
			msgClass.intIndex = 693;
			msgClass.MessageString = this.messageString;
			
			while(msgClass.intIndex < msgClass.MessageString.length())
			{
				TC1236_KNTRSK35DS msg = new TC1236_KNTRSK35DS(this.getContext());
				
				msg.setIPKIPN(msgClass.getMessageNumber(19,0));
				msg.setIPKRTN(msgClass.getMessageNumber(19,0));
				msg.setKRSVID(msgClass.getMessageString(6));
				msg.setWDSVID(msgClass.getMessageString(25));
				msg.setKRRKID(msgClass.getMessageString(6));
				msg.setWDRKID(msgClass.getMessageString(25));
				msg.setANATP1(msgClass.getMessageString(1));
				msg.setKRAVST(msgClass.getMessageNumber(3,0, true));
				msg.setKRKNST(msgClass.getMessageNumber(3,0, true));
				msg.setGIAVST(msgClass.getMessageNumber(3,0, true));
				msg.setKTEMNM(msgClass.getMessageString(26));
				msg.setIPNAME(msgClass.getMessageString(40));
				msg.setLLORGA(msgClass.getMessageNumber(15,2));
				msg.setKRSLDO(msgClass.getMessageNumber(15,2));
				msg.setKRPRBO(msgClass.getMessageNumber(15,2));
				msg.setUPPTOT(msgClass.getMessageNumber(15,2));
				msg.setPHBELP(msgClass.getMessageNumber(15,2));
				msg.setKRIBDT(msgClass.getMessageNumber(8,0));
				msg.setKRKNTN(msgClass.getMessageNumber(19,0));
				msg.setKRGACC(msgClass.getMessageNumber(19,0));
				msg.setKRUSLD(msgClass.getMessageNumber(15,2));
				msg.setKRLBET(msgClass.getMessageNumber(15,2));
				msg.setKRKAID(msgClass.getMessageString(3));
				msg.setKRPGRP(msgClass.getMessageString(3));
				msg.setLLINNB(msgClass.getMessageNumber(5,0));
				msg.setKRBCDE(msgClass.getMessageString(3));
				msg.setWDFLAG(msgClass.getMessageString(1));
				msg.setSKSPRK(msgClass.getMessageString(2));
				msg.setKRBKRD(msgClass.getMessageNumber(11,0));
				msg.setWDCAVL(msgClass.getMessageNumber(15,2));
				msg.setWDAAVL(msgClass.getMessageNumber(15,2));
				msg.setNBRI(msgClass.getMessageNumber(3,0));
				msg.setDATE(msgClass.getMessageNumber(7,0));
				msg.setUPAVDT(msgClass.getMessageNumber(8,0));
				msg.setUPDUDT(msgClass.getMessageNumber(8,0));
				msg.setUPSEQN(msgClass.getMessageNumber(5,0));
				msg.setUPAAMT(msgClass.getMessageNumber(15,2));
				msg.setWDTOTD(msgClass.getMessageNumber(15,2));
				msg.setINBEL(msgClass.getMessageNumber(15,2));
				msg.setINBDAT(msgClass.getMessageNumber(8,0));
				
				msg.setSearchKey(searchKey);
				msg.setMoreRecord(moreRecord);
				msg.setResponseCode("OK");
				msg.setResponseDescription("Successfully.");
				
				list.add(msg.getMendixObject());

			}
		}
		catch (Exception ex)
		{
			TC1236_KNTRSK35DS msg = new TC1236_KNTRSK35DS(this.getContext());
			msg.setResponseCode("ER");
			msg.setResponseDescription(ex.getMessage() + " [intIndex=" + msgClass.intIndex + ", Length=" + msgClass.MessageString.length() + "]");
			
			list.add(msg.getMendixObject());
		}
		
		return list;
		// END USER CODE
	}

	/**
	 * Returns a string representation of this action
	 */
	@java.lang.Override
	public java.lang.String toString()
	{
		return "GetTC1236";
	}

	// BEGIN EXTRA CODE
	// END EXTRA CODE
}
