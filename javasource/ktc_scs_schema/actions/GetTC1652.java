// This file was generated by Mendix Studio Pro.
//
// WARNING: Only the following code will be retained when actions are regenerated:
// - the import list
// - the code between BEGIN USER CODE and END USER CODE
// - the code between BEGIN EXTRA CODE and END EXTRA CODE
// Other code you write will be lost the next time you deploy the project.
// Special characters, e.g., é, ö, à, etc. are supported in comments.

package ktc_scs_schema.actions;

import com.mendix.systemwideinterfaces.core.IContext;
import com.mendix.systemwideinterfaces.core.IMendixObject;
import com.mendix.webui.CustomJavaAction;
import ktc_scs_schema.proxies.TC1652_KNTRSK22DS;
import ktcmdwscsconnector.customs.MessageClass;

public class GetTC1652 extends CustomJavaAction<java.util.List<IMendixObject>>
{
	private java.lang.String messageString;
	private java.lang.String searchKey;
	private java.lang.String moreRecord;

	public GetTC1652(IContext context, java.lang.String messageString, java.lang.String searchKey, java.lang.String moreRecord)
	{
		super(context);
		this.messageString = messageString;
		this.searchKey = searchKey;
		this.moreRecord = moreRecord;
	}

	@java.lang.Override
	public java.util.List<IMendixObject> executeAction() throws Exception
	{
		// BEGIN USER CODE
		MessageClass msgClass = new MessageClass();
		java.util.List<IMendixObject> list = new java.util.ArrayList<IMendixObject>();
		
		try
		{
			// Start Detail
			msgClass.intIndex = 693;
			msgClass.MessageString = this.messageString;
			
			while(msgClass.intIndex < msgClass.MessageString.length())
			{
				TC1652_KNTRSK22DS msg = new TC1652_KNTRSK22DS(this.getContext());
				
				msg.setKRKNTN(msgClass.getMessageNumber(19, false));
				msg.setKRRKID(msgClass.getMessageString(6));
				msg.setANOTYP(msgClass.getMessageString(6));
				msg.setKRKNST(msgClass.getMessageNumber(3, true));
				msg.setKRBCDE(msgClass.getMessageString(3));
				msg.setKRAVST(msgClass.getMessageNumber(3, true));
				msg.setKRUPDT(msgClass.getMessageNumber(8, false));
				msg.setKRPDAY(msgClass.getMessageNumber(2, false));
				msg.setKRBKRD(msgClass.getMessageNumber(11, false));
				msg.setWDOPTB(msgClass.getMessageNumber(15, 2));
				msg.setKRSLDO(msgClass.getMessageNumber(15, 2));
				msg.setWDDDBF(msgClass.getMessageString(1));
				msg.setKRLMTF(msgClass.getMessageString(1));
				msg.setWDDCDE(msgClass.getMessageString(48));
				msg.setWDPCAT(msgClass.getMessageString(2));
				msg.setKRCBIL(msgClass.getMessageString(1));
				msg.setKTKRTN(msgClass.getMessageNumber(19, false));
				msg.setKTEMDT(msgClass.getMessageNumber(8, true));
				msg.setKRATP1(msgClass.getMessageString(1));
				msg.setIPKIPN(msgClass.getMessageNumber(19, false));
				msg.setIPOLIC(msgClass.getMessageString(40));
				msg.setIPITP1(msgClass.getMessageString(3));
				
				msg.setResponseCode("OK");
				msg.setResponseDescription("Successfully.");
				msg.setSearchKey(this.searchKey);
				msg.setMoreRecord(moreRecord);
				
				if (!msg.getKRRKID().toString().equals(""))
				{
					list.add(msg.getMendixObject());
				}
			}
		}
		catch (Exception ex)
		{
			TC1652_KNTRSK22DS msg = new TC1652_KNTRSK22DS(this.getContext());
			msg.setResponseCode("ER");
			msg.setResponseDescription(ex.getMessage() + " [intIndex=" + msgClass.intIndex + ", Length=" + msgClass.MessageString.length() + "]");
			msg.setSearchKey(this.searchKey);
			list.add(msg.getMendixObject());
		}
		
		return list;
		// END USER CODE
	}

	/**
	 * Returns a string representation of this action
	 */
	@java.lang.Override
	public java.lang.String toString()
	{
		return "GetTC1652";
	}

	// BEGIN EXTRA CODE
	// END EXTRA CODE
}
